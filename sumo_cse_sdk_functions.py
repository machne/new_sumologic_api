...
def download_rules_inventory(access_id, access_key, endpoint, rule_types_list, org_name):
    try:   # Create filename with customer name and current timestamp
        
        sumo = SumoLogic(access_id, access_key, endpoint)
        
        now = datetime.now()
        file_name = f"{org_name}-{now.strftime('%Y%m%d-%H%M')}.xlsx"
        console.log(f'File name: {file_name}')
        console.log(endpoint)
        # Define rule types and query
        console.log(f"Building Query for {org_name} ...")
        ...
        safe_sheet_name = clean_sheet_name(org_name)
        ...
        console.log(f"Successfully wrote data for {org_name} to: {file_name}")
